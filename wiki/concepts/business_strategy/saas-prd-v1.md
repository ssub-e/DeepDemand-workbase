...
| **Could** | **(F4) 악성 재고 위험 신호등 & 보관료 누수 경고** | 과발주 방지 보조 및 홈/리빙 버티컬의 창고 체류 시간에 따른 보관료 비용 역산 가치 제공. |
...
### 7-5. 버티컬 지원 범위 (Vertical Domain Coverage)
SaaS 피벗 및 범용성 강화를 위해 본 MVP는 5대 주요 버티컬 도메인의 고유 요구사항과 AI 맞춤 예측 방식을 정합적으로 지원한다.
*   **패션 / 의류**: 다옵션(색상, 사이즈)의 엑셀 데이터 파싱 보강 및 스파이크 감지/동대문 사입 리드타임 최적화를 통해 특정 옵션의 악성 재고를 원천 차단.
*   **코스메틱 / 뷰티**: 다채널 분산 판매 상황에서 인플루언서 노출 등에 의한 급격한 수요 급증(Demand Spike)에 선제 대응.
*   **신선식품 / 푸드**: 유통기한 및 온도 민감성을 감안하여 실시간 기상 API 연동 및 예측 데이터를 통해 폐기 누수를 차단하는 안전재고 안전장치.
*   **홈 / 리빙 / 잡화**: 보관 공간을 많이 차지하는 특성을 고려해 장기 체류 시간에 따른 창고 보관료 비용과 품절 랭킹 하락 리스크를 역산하여 '최적 입고량' 제안 (현금 유동성 30% 개선).
*   **초단기 트렌드 / 공동구매**: 판매 이력이 전무한 신상품 론칭 시, 14일 미만의 경우 규칙 기반(Rule-based)의 초기 수요 벤치마크 및 소셜 반응 가중치 Fallback 시스템 가동.
